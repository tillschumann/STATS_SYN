...
public:
  ...
  void setFirstSyn(uint64_t first)
  {
    global_offset_ = first;
    
    //std::cout << "global_offset_=" << global_offset_ << std::endl;
  }
  ...
  void setLastSyn(uint64_t last)
  {
    ...
    //std::cout << "total_num_syns_=" << total_num_syns_ << std::endl;
  }
  ...
  {    
    ...
    H5Tinsert (memtype, "delay", HOFFSET (NESTNodeSynapse, delay), H5T_NATIVE_FLOAT);
    H5Tinsert (memtype, "weight", HOFFSET (NESTNodeSynapse, weight), H5T_NATIVE_FLOAT);
    H5Tinsert (memtype, "U0", HOFFSET (NESTNodeSynapse, U0), H5T_NATIVE_FLOAT);
    H5Tinsert (memtype, "TauRec", HOFFSET (NESTNodeSynapse, TauRec), H5T_NATIVE_FLOAT);
    H5Tinsert (memtype, "TauFac", HOFFSET (NESTNodeSynapse, TauFac), H5T_NATIVE_FLOAT);
    ...
    nest::Stopwatch::timestamp_t before_read=nest::Stopwatch::get_timestamp();
    
    H5Dread (syn_dataset.getId(), memtype, memspace_id, dataspace_id, dxpl_id_, &synapses[0]);
    
    nest::Stopwatch::timestamp_t dur_read=nest::Stopwatch::get_timestamp()-before_read;
    
    std::cout << "read\t" << RANK << "\t" << dur_read/1000 << std::endl;
